...
<!--                <span id="progress" class="loader"></span>-->
        ...
        `,(o=document.querySelector("#submit"))==null||o.addEventListener("click",()=>{this.completePayment(e)})}async createPaymentIntent(e){fetch(`${l}/aion/payments/config`).then(t=>t.json()).then(t=>{I(t.stripe_public_key).then(n=>{this.stripe=n,this.is_test_mode=t.stripe_public_key.includes("test"),fetch(`${l}/aion/payments/create_intent`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${this.checkUserToken()}`},body:JSON.stringify({product_id:this.is_test_mode?e.test_id:e.id})}).then(s=>s.json()).then(s=>{var i;if(s.code===10)throw console.warn(s.message),alert(s.message),window.location.href=`https://${window.location.host}/`,new Error(s.message);this.elements=this.stripe.elements({clientSecret:s.client_secret});let o=this.elements.create("payment");(i=document.querySelector("#progress-loader"))==null||i.classList.add("hide"),document.querySelector("#submit").style.visibility="visible",o.mount("#payment-element")}).catch(s=>{console.error("Something wrong when create a payment intent",s),alert("Something wrong when create a payment. Please try again.")})})})}checkUserToken(){let e=localStorage.getItem("_ms-mid");if(!e)throw new Error("User token not found");return e}async initPaymentForm(e){console.log("Init payment form...");let t=await this.initStripe();if(this.stripe=t,!t)return;let n={theme:"flat",variables:{colorPrimary:"#30313d",colorText:"#30313d"},roles:{".TermsText":{hide:!0}}},s={},o="auto",i=await this.getClientSecret(e);if(!i)return;let r=t.elements({clientSecret:i,appearance:n,loader:o});this.elements=r;let u=r.create("payment",s).mount("#payment-element")}async getClientSecret(e){return fetch(`${l}/aion/payments/create_intent`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${this.checkUserToken()}`},body:JSON.stringify({product_id:this.is_test_mode?e.test_id:e.id})}).then(t=>t.json()).then(t=>{if(console.log(t),t.code===10){let n=this.is_test_mode?e.test_id:e.id;if(console.log(n,t.plan_id),t.plan_id&&t.plan_id!==n)return this.updateSubscription(e);throw console.warn(t.message),alert(t.message),window.location.href=`https://${window.location.host}/`,new Error(t.message)}return document.getElementById("button-text").innerHTML=`$${t.amount}`,t.client_secret})}async updateSubscription(e){return console.log("Update subscription..."),fetch(`${l}/aion/payments/subscription`,{method:"PUT",headers:{"Content-Type":"application/json",Authorization:`Bearer ${this.checkUserToken()}`},body:JSON.stringify({product_id:this.is_test_mode?e.test_id:e.id})}).then(t=>t.json()).then(t=>{var n;if(console.log(t),t.credit){(n=document.querySelector("#payment-form"))==null||n.classList.add("hide"),G(t.credit,e.name);return}return document.getElementById("button-text").innerHTML=`Pay $${t.amount}`,t.client_secret}).catch(t=>{console.error("Something wrong when update a subscription",t),alert("Something wrong when update a subscription. Please try again or contact us."),window.location.href=`https://${window.location.host}/#plans`})}initStripe(){return console.log("Init stripe..."),fetch(`${l}/aion/payments/config`).then(e=>e.json()).then(e=>(this.is_test_mode=e.stripe_public_key.includes("test"),I(e.stripe_public_key).then(t=>t))).catch(e=>{console.error("Something wrong when init stripe",e)})}async completePayment(e){console.log("Complete payment...");let t=document.querySelector("#submit");if(t.innerHTML.includes("Payments attempt")){window.location.href=`https://${window.location.host}/#plans`;return}let n=t.innerHTML,s=null,o=null;for(let i=1;i<=5&&(this.showBlinkMessage(`Payments attempt ${i}...`,t),s=await this.stripe.confirmPayment({elements:this.elements,confirmParams:{return_url:`https://${window.location.host}/dashboard/settings?payment_success=${e.name}`}}).then(r=>r.error?(o=r.error.message,"error"):(console.log(r),"success")),t.innerHTML=n,console.log("result: ",s),!(s==="false"||s==="success"));i++);console.log(s,o),s==="error"&&alert(o)}async cancelSubscription(){fetch(`${l}/aion/payments/cancel_subscription`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${this.checkUserToken()}`}}).then(e=>e.json()).catch(e=>{console.error("Something wrong when create a checkout session",e)})}async getSubscriptionInfo(){try{let e=await fetch(`${l}/aion/payments/subscription`,{method:"GET",headers:{"Content-Type":"application/json",Authorization:`Bearer ${this.checkUserToken()}`}});return await this.handleResponse(e)}catch{return null}}async handleResponse(e){if(!e.ok){let t=await e.json();throw{status:e.status,message:t}}if(e.status!==204)return await e.json()}isValidCreditCardNumber(e){if(e=e.replace(/\s/g,"").split("").reverse().join(""),!/^[0-9]{13,19}$/.test(e))return!1;let t=0;for(let n=0;n<e.length;n++){let s=parseInt(e[n]);n%2===1&&(s*=2,s>9&&(s-=9)),t+=s}return t%10===0}};var X=new L,d=class{constructor(e){let t=c.getToken(),n=`${l}/aion/${e}`;this.client=new v(n,t)}static sendVerificationEmail(){let{client:e}=d.getInstance();return e.get("email_verification",{})}static getInstance(e="users"){return d.instance||(d.instance=new d(e)),d.instance}static async fetchRequests(e=0,t=10){try{let{client:n}=d.getInstance(),s=`data?filters=requests&offset=${e}&limit=${t}`;return await n.get(s).then(o=>o.requests.array)}catch(n){return console.error("getRequests:",n),[]}}static async fetchUsageApi(){try{let{client:e}=d.getInstance(),t="data?filters=all&offset=0&limit=1";return await e.get(t).then(n=>n.api)}catch(e){return console.error("fetchUsageApi:",e),[]}}static async signUp(){try{let{client:e}=d.getInstance();return await e.post("sign_up",{}).then(()=>!1).catch(t=>{throw t})}catch(e){throw console.error("signUp:",e),e}}static async login(){try{let{client:e}=d.getInstance();return await e.get("login")}catch(e){console.error("login:",e)}}static async delete(){try{let{client:e}=d.getInstance();return c.removeAuth(),await e.delete(""),!0}catch(e){console.error("delete:",e)}}static async fetchApiToken(){try{let{client:e}=d.getInstance();return await e.post("api_token",{})}catch(e){console.error("fetchApiToken:",e),console.error("fetchApiToken:",e)}}static async refreshApiToken(){try{let{client:e}=d.getInstance();return await e.patch("api_token",{})}catch(e){console.error("refreshApiToken:",e)}}static async fetchSubscriptionData(){let{client:e}=d.getInstance();try{let t="data?filters=all&offset=0&limit=1";return await e.get(t).then(n=>n)}catch(t){console.error("fetchSubscriptionData:",t)}}static async subscriptionInfo(){var r,u,S,x,C,B,A,M,H,$,U,R,z,V,D;let e=document.querySelector("#plan-info"),t=document.querySelector("#usage-info"),n=document.querySelector("#btn-cancel-plan");e.classList.add("hide"),t.classList.add("hide"),n.classList.add("hide");let s=[d.fetchSubscriptionData(),X.getSubscriptionInfo()],[o,i]=await Promise.all(s);if(o!=null&&o.plan||(e.innerHTML=`You're on the <span class="text-color-green">Free</span> plan. You have limits of 20 & 100 checks per month via web & API, respectively. If you need more, check out our <a class="text-color-green" href='https://${window.location.host}/#plans' >plans</a>.`,t.innerHTML=`You have used ${((r=o==null?void 0:o.requests)==null?void 0:r.total)||0} of 20 checks via web and ${((S=(u=o==null?void 0:o.api)==null?void 0:u.usage)==null?void 0:S.daily)||0} of 100 checks via API.`,e.classList.remove("hide"),t.classList.remove("hide")),(o==null?void 0:o.plan.requests_limits.quantity)<=1e3){let k=o==null?void 0:o.plan.requests_limits.quantity;e.innerHTML=`You're on the <span class="text-color-green">Base</span> plan. You have limits of ${k} requests for both web & API.`,t.innerHTML=`You have used ${(((x=o==null?void 0:o.requests)==null?void 0:x.total)||0)+(((B=(C=o==null?void 0:o.api)==null?void 0:C.usage)==null?void 0:B.daily)||0)} of ${k} checks via both web API.`,(A=i==null?void 0:i.subscription.meta)!=null&&A.was_canceled?(n.classList.add("hide"),t.innerHTML=t.innerHTML+` Your subscription has been canceled, expires on ${new Date(i==null?void 0:i.subscription.expiration_dt).toLocaleDateString()}.`):n.classList.remove("hide"),e.classList.remove("hide"),t.classList.remove("hide")}(o==null?void 0:o.plan.requests_limits.quantity)===1e4&&(e.innerHTML=`You're on the <span class="text-color-green">PRO</span> plan. You have limits of 10000 requests for both web & API.`,t.innerHTML=`You have used ${(((M=o==null?void 0:o.requests)==null?void 0:M.total)||0)+((($=(H=o==null?void 0:o.api)==null?void 0:H.usage)==null?void 0:$.daily)||0)} of 10000 checks via both web & API.`,(R=(U=i==null?void 0:i.subscription)==null?void 0:U.meta)!=null&&R.was_canceled?(n.classList.add("hide"),t.innerHTML=t.innerHTML+` Your subscription has been canceled, expires on ${new Date(i==null?void 0:i.subscription.expiration_dt).toLocaleDateString()}.`):n.classList.remove("hide"),e.classList.remove("hide"),t.classList.remove("hide")),(o==null?void 0:o.plan.requests_limits.quantity)>1e4&&(e.innerHTML=`You're on the <span class="text-color-green">Custom</span> plan. You have limits of ${o==null?void 0:o.plan.requests_limits.quantity} requests for both web & API.`,t.innerHTML=`You have used ${(((z=o==null?void 0:o.requests)==null?void 0:z.total)||0)+(((D=(V=o==null?void 0:o.api)==null?void 0:V.usage)==null?void 0:D.daily)||0)} of 10000 checks via both web & API.`,n.innerHTML="Contact US  to update your plan.",n.classList.remove("hide"),e.classList.remove("hide"),t.classList.remove("hide")),n.addEventListener("click",async()=>{(o==null?void 0:o.plan.requests_limits.quantity)>1e4?window.location.href=`https://${window.location.host}/contact-us`:confirm("Are you sure you want to cancel your subscription?")?(alert("Your subscription has been canceled."),X.cancelSubscription().then(k=>{console.log(k)}),window.location.href=`https://${window.location.host}/#plans`):alert("Your subscription has not been canceled.")})}static async getUserInfo(){let{client:e}=d.getInstance();try{return await e.get("")}catch(t){console.error("getUserInfo:",t)}}static async getTransactions(){let{client:e}=new d("payments");try{return await e.get("invoices")}catch(t){console.error("getTransactions:",t)}}},m=d;m.instance=null;var y=class{constructor(){}static getUserInfo(){return m.getUserInfo()}static isAuth(){return localStorage.getItem(y.token_key)!==null}static setAuth(){localStorage.setItem(y.key,"true")}static removeAuth(){localStorage.removeItem(y.key)}static async init(){if(localStorage.getItem("_aion_in")===null)try{await m.signUp(),y.setAuth(),await m.login(),localStorage.setItem("_aion_in","true")}catch(e){console.log(e)}}static getToken(){var e;return(e=localStorage.getItem("_ms-mid"))!=null?e:""}static isExpiredToken(){let e=y.getToken();if((e==null?void 0:e.length)>0){let t=j(e),n=Date.now()/1e3;return t.exp<n}return!0}static checkAuth(e){if(!y.isAuth()){let t=document.getElementById("sign-up");return t.style.display="flex",t.style.zIndex=100,e(),!0}return!1}static async sendVerifiedEmail(){console.log("sendVerifiedEmail"),await m.sendVerificationEmail()}},c=y;c.key="isSignUp",c.token_key="_ms-mid";var p=class{constructor(){let e=c.getToken(),t=`${l}/aion/ai-generated`;this.client=new v(t,e)}static getInstance(){return p.instance||(p.instance=new p),p.instance}static async getReportsByBinary(e){let{client:t}=p.getInstance();try{let n=new FormData;return n.append("binary",e,"uploaded-file.png"),await t.postBinary("reports/binary",n)}catch(n){n.status===402&&alert("Please verify your email to continue using the service"),n.status===429&&(alert(`It looks like you have reached your plan limit of ${n.message.msg.current_limit} requests. To continue, please upgrade to a new plan.`),window.location.href=`https://${window.location.host}/#plans`),console.error("Error getReportsByBinary:",n)}}static async getReportsByUrl(e){let{client:t}=p.getInstance();try{let n=`reports/url?url=${e}`;return await t.post(n,{})}catch(n){n.status===402&&alert("Please verify your email to continue using the service"),n.status===429&&(alert(`It looks like you have reached your plan limit of ${n.message.msg.current_limit} requests. To continue, please upgrade to a new plan.`),window.location.href=`https://${window.location.host}/#plans`),console.error("getReportsByUrl:",n)}}static async getAudioVerdict(e){let{client:t}=p.getInstance();try{let n=new FormData;return n.append("file",e),await t.postBinary("reports/audio/binary",n)}catch(n){n.status===402&&alert("Please verify your email to continue using the service"),n.status===429&&(alert(`It looks like you have reached your plan limit of ${n.message.msg.current_limit} requests. To continue, please upgrade to a new plan.`),window.location.href=`https://${window.location.host}/#plans`),console.error("Error getAudioVerdict:",n)}}static async getYoutubeVerdict(e){let{client:t}=p.getInstance();try{let n={url:e};return await t.post("reports/audio/link",n)}catch(n){n.status===402&&alert("Please verify your email to continue using the service"),n.status===429&&(alert(`It looks like you have reached your plan limit of ${n.message.msg.current_limit} requests. To continue, please upgrade to a new plan.`),window.location.href=`https://${window.location.host}/#plans`),console.error("Error getYoutubeVerdict:",n)}}static async getPdetReportByUrl(e){let{client:t}=p.getInstance();try{let n={object:e};return await t.post("reports/person_detection/url",n)}catch(n){n.status===402&&alert("Please verify your email to continue using the service"),n.status===429&&(alert(`It looks like you have reached your plan limit of ${n.message.msg.current_limit} requests. To continue, please upgrade to a new plan.`),window.location.href=`https://${window.location.host}/#plans`),console.error("Error getYoutubeVerdict:",n)}}static async getPdetReportByBinary(e){let{client:t}=p.getInstance();try{let n=new FormData;return n.append("file",e,"uploaded-file.png"),await t.postBinary("reports/person_detection/binary",n)}catch(n){n.status===402&&alert("Please verify your email to continue using the service"),n.status===429&&(alert(`It looks like you have reached your plan limit of ${n.message.msg.current_limit} requests. To continue, please upgrade to a new plan.`),window.location.href=`https://${window.location.host}/#plans`),console.error("Error getReportsByBinary:",n)}}},h=p;h.instance=null;var f=class{constructor(){}static async getReportsByBinary(e,t){let n=`${l}/results/api/detector/reports/raw?source=web&user_id=${t}`,s=new FormData;s.append("binary",e,"file_name.png");let o={method:"POST",headers:{Accept:"application/json",Authorization:`Bearer ${c.getToken()}`},body:s};return await fetch(n,o).then(i=>i.json())}static async getReportsByUrl(e,t){let n=`${l}/results/api/detector/reports/json?source=web&user_id=${t}`,s={method:"POST",headers:{Accept:"application/json","Content-Type":"application/json",Authorization:`Bearer ${c.getToken()}`},body:JSON.stringify({object:e})};return await fetch(n,s).then(o=>o.json())}static async sendFeedback(e,t,n,s=!1){let o={is_proper_predict:t,comment:n},i=`${l}/results/api/detector/reports/result/${e}`,r={method:"PUT",body:JSON.stringify(o),headers:{Accept:"application/json","Content-Type":"application/json"}};(s||!c.isExpiredToken())&&(i=`${l}/aion/ai-generated/reports/${e}`,r={method:"PATCH",body:JSON.stringify(o),headers:{Accept:"application/json","Content-Type":"application/json"}}),await fetch(i,r).then(u=>u.json()).then(u=>console.log(u)).catch(u=>console.error(u))}static async getAudioVerdict(e){let t=`${l}/aion/ai-generated/reports/audio/binary`,n=new FormData;return n.append("file",e),await fetch(t,{method:"POST",headers:{Accept:"application/json",ContentType:"multipart/form-data"},body:n}).then(o=>o.json())}static async getYoutubeVerdict(e){let t=`${l}/aion/ai-generated/reports/audio/link`,n={method:"POST",headers:{Accept:"application/json","Content-Type":"application/json"},body:JSON.stringify({url:e})};return await fetch(t,n).then(s=>s.json())}};var q=class{constructor(){}static isLimitExceeded(){return!!c.isExpiredToken()}static increment(){let e=localStorage.getItem(q.key),t=e===null?1:Number(e)+1;localStorage.setItem(q.key,t.toString())}},b=q;b.key="requestCount";var g=class{static async getReportsByBinary(e,t){return c.isExpiredToken()?await f.getReportsByBinary(e,t):await h.getReportsByBinary(e)}static async getReportsByUrl(e,t){return c.isExpiredToken()?await f.getReportsByUrl(e,t):await h.getReportsByUrl(e)}static async getAudioVerictByFile(e){return await h.getAudioVerdict(e)}static async getAudioVerictMock(e){let n=await((s,o)=>new Promise(i=>{setTimeout(()=>{i(o)},s)}))(1500,e);return JSON.parse(`{
            ...
            "report": {
                ...
                `,r.style.width="100%",r.style.height="100%",r.style.display="flex",r.style.justifyContent="center",r.style.alignItems="center",o.appendChild(r)}else{let r=document.createElement("img");r.src=s.url,r.alt=s.verdict,o.appendChild(r)}o.appendChild(i),n.appendChild(o)}))}static fillApiKeyCard(e){let t=S=>{let x=new Date(S),C={month:"short",day:"numeric",year:"numeric",hour:"numeric",minute:"numeric"};return x.toLocaleDateString("en-US",C)},n=document.getElementById("api-item"),s=document.getElementById("expire-date"),o=document.getElementById("rps"),i=document.getElementById("progress-line"),r=document.getElementById("counter-requests"),u=document.getElementById("total-requests");n&&s&&o&&i&&r&&u&&e.expiration_dt&&(s.innerText=t(e.expiration_dt),o.innerText=e.limits.secondly.toString(),i.style.width=`${e.usage.daily/e.limits.daily*100}%`,r.innerText=e.usage.daily.toString(),u.innerText=e.limits.daily.toString(),n.style.display="flex")}static fillCardControls(e,t){let n=document.createElement("button");if(n.onclick=()=>{n.innerText="Copied!",T(`https://results.aiornot.com/aiornot/users/${t.id}`),setTimeout(()=>{n.innerText="Share"},1500)},n.innerText="Share",n.classList.add("request-item-share"),n.style.opacity="0",!t.hasOwnProperty("is_proper_predict")){let s=document.createElement("div");s.id="request-item-controls",s.style.display="none";let o=document.createElement("button");o.innerHTML=`
            ...
            `,i.classList.add("request-item-dislike"),s.appendChild(o),s.appendChild(i),e.appendChild(s),o.onclick=()=>{let r=document.createElement("div");r.classList.add("feedback-alert"),r.classList.add("feedback-alert__correct"),r.innerHTML=`
                          ...
                          `,s.remove(),e.appendChild(r),setTimeout(()=>{r.remove()},5e3),g.sendFeedback(t.id,!0,"")},i.onclick=()=>{let r=document.createElement("div");r.classList.add("feedback-alert"),r.classList.add("feedback-alert__incorrect"),r.innerHTML=`
                              ...
                              <span>Incorrect</span>
                          `,s.remove(),e.appendChild(r),setTimeout(()=>{r.remove()},5e3),g.sendFeedback(t.id,!1,"")}}e.appendChild(n)}};async function Se(){let a=document.getElementById("refresh-api-key"),e=document.getElementById("request-api-key"),t=document.getElementById("dash-api-empty"),n=document.getElementById("dash-api-content"),s=document.getElementById("api-copy");s.classList.add("hide"),await o(),a.onclick=async()=>{let{token:r}=await m.refreshApiToken();i(r)},e.onclick=async()=>{let{token:r}=await m.fetchApiToken();await o(r)};async function o(r=null){let u=await m.fetchUsageApi();u.access?(t.style.display="none",n.style.display="block",E.fillApiKeyCard(u)):(t.style.display="flex",n.style.display="none"),i(r)}function i(r=null){if(console.log("token",r),r){if(s.classList.remove("hide"),!s)return;s.onclick=()=>{T(r),s.classList.add("hide")}}}}document.getElementById("sign-out").onclick=()=>{localStorage.removeItem("_aion_in"),c.removeAuth()};Se();})();
