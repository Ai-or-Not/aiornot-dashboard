...
@keyframes fadeout {
  from {top: 30px; opacity: 1;}
  to {top: 0; opacity: 0;}
`,j=document.createElement("style");j.innerHTML=Y;document.head.appendChild(j);var w=document.getElementById("toast");function R(i,e="info"){w.classList.remove("hide");let t=document.createElement("span");t.innerHTML="x",t.className="close-button",t.addEventListener("click",()=>{w.classList.add("hide"),e!=="error"&&(window.location.href=`https://${window.location.host}/dashboard/settings`)}),w.innerHTML=i,w.appendChild(t),w.classList.add("show"),w.style.borderColor=e==="error"?"red":"#aefc06"}var D=i=>{R(`<p style="font-size: 1.5rem; padding: 2rem;">Amazing! You\u2019ve just upgraded your account, welcome to the exclusive ranks of AI or Not <span style="color: #aefc06; font-weight: bold ">${i}</span> users. Thank you for being a supporter of AI or Not and a purveyor of the truth!</p>`)},O=(i,e)=>{R(`<p style="font-size: 1.5rem; padding: 2rem;">We are pleased to inform you that you currently have a credit of $${i} from a previous paid subscription after that we successfully update your plan to <span style="color: #aefc06; font-weight: bold ">${e.split(" ")[0]}</span>! If you have any questions please contact us.</p>`)};function q(i){var e=i.split(".")[1],t=e.replace(/-/g,"+").replace(/_/g,"/"),n=decodeURIComponent(atob(t).split("").map(function(r){return"%"+("00"+r.charCodeAt(0).toString(16)).slice(-2)}).join(""));return JSON.parse(n)}var M="https://js.stripe.com/v3",J=/^https:\/\/js\.stripe\.com\/v3\/?(\?.*)?$/,N="loadStripe.setLoadParameters was called but an existing Stripe.js script already exists in the document; existing script parameters will be used",G=function(){for(var e=document.querySelectorAll('script[src^="'.concat(M,'"]')),t=0;t<e.length;t++){var n=e[t];if(J.test(n.src))return n}return null},W=function(e){var t=e&&!e.advancedFraudSignals?"?advancedFraudSignals=false":"",n=document.createElement("script");n.src="".concat(M).concat(t);var r=document.head||document.body;if(!r)throw new Error("Expected document.body not to be null. Stripe.js requires a <body> element.");return r.appendChild(n),n},X=function(e,t){!e||!e._registerWrapper||e._registerWrapper({name:"stripe-js",version:"2.1.7",startTime:t})},E=null,K=function(e){return E!==null||(E=new Promise(function(t,n){if(typeof window=="undefined"||typeof document=="undefined"){t(null);return}if(window.Stripe&&e&&console.warn(N),window.Stripe){t(window.Stripe);return}try{var r=G();r&&e?console.warn(N):r||(r=W(e)),r.addEventListener("load",function(){window.Stripe?t(window.Stripe):n(new Error("Stripe.js not available"))}),r.addEventListener("error",function(){n(new Error("Failed to load Stripe.js"))})}catch(o){n(o);return}})),E},Q=function(e,t,n){if(e===null)return null;var r=e.apply(void 0,t);return X(r,n),r},z=Promise.resolve().then(function(){return K(null)}),H=!1;z.catch(function(i){H||console.warn(i)});var U=function(){for(var e=arguments.length,t=new Array(e),n=0;n<e;n++)t[n]=arguments[n];H=!0;var r=Date.now();return z.then(function(o){return Q(o,t,r)})};var Z=window.location.host.includes("webflow")?"stage":"prod",m=`https://${Z}.ai-or-not.com`;var k=class{constructor(e,t){this.apiUrl=e,this.bearerToken=t}async get(e){let t=`${this.apiUrl}/${e}`;try{let n=await fetch(t,{method:"GET",headers:{"Content-Type":"application/json",Authorization:`Bearer ${this.bearerToken}`}});return await this.handleResponse(n)}catch(n){throw console.error("Error",n),n}}async post(e,t){let n=`${this.apiUrl}/${e}`;try{let r=await fetch(n,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${this.bearerToken}`},body:JSON.stringify(t)});return await this.handleResponse(r)}catch(r){throw console.error("Request error",r),r.status===429&&r.message.msg.type==="requests"&&alert(`It looks like you have reached your plan limit of ${r.message.current_limit} requests. To continue, please upgrade to a new plan.`),r}}async postBinary(e,t){let n=`${this.apiUrl}/${e}`;try{let r=await fetch(n,{method:"POST",headers:{Accept:"application/json",Authorization:`Bearer ${this.bearerToken}`},body:t});return await this.handleResponse(r)}catch(r){throw console.error("Binary request error:",r),r.status===429&&r.message.msg.type==="requests"&&alert(`It looks like you have reached your plan limit of ${r.message.current_limit} requests. To continue, please upgrade to a new plan.`),r}}async delete(e){let t=`${this.apiUrl}/${e}`;try{let n=await fetch(t,{method:"DELETE",headers:{accept:"*/*",Authorization:`Bearer ${this.bearerToken}`}});await this.handleResponse(n)}catch(n){throw console.error("Error when perform DELETE-request:",n),n}}async patch(e,t){let n=`${this.apiUrl}/${e}`;try{let r=await fetch(n,{method:"PATCH",headers:{"Content-Type":"application/json",Authorization:`Bearer ${this.bearerToken}`},body:JSON.stringify(t)});return await this.handleResponse(r)}catch(r){throw console.error("\u041E\u0448\u0438\u0431\u043A\u0430 \u043F\u0440\u0438 \u0432\u044B\u043F\u043E\u043B\u043D\u0435\u043D\u0438\u0438 PATCH-\u0437\u0430\u043F\u0440\u043E\u0441\u0430:",r),r}}async handleResponse(e){if(!e.ok){let t=await e.json();throw{status:e.status,message:t}}if(e.status!==204)return await e.json()}};var F=`
...
`;function S(d){var x;let a=(x=document.getElementById("table"))==null?void 0:x.getElementsByTagName("tbody")[0],c=a==null?void 0:a.insertRow(a.rows.length),T=c==null?void 0:c.insertCell(0),$=c==null?void 0:c.insertCell(1),f=c==null?void 0:c.insertCell(2),_=c==null?void 0:c.insertCell(3),I=c==null?void 0:c.insertCell(4);T.innerHTML=`<p class="cell"> ${d.num} </p>`,$.innerHTML=`<p class="cell"> ${d.name} </p>`,f.innerHTML=`<p class="cell"> ${d.amount} </p>`,_.innerHTML=`<p class="cell" style="color: ${d.status==="paid"?"#00D924":"red"}" > ${d.status} </p>`,d.status!=="draft"&&(I.innerHTML=`<a class="cell" href="${d.invoice_link}" target="_blank" style="text-decoration: underline"> Invoice </a>`)}u.getTransactions().then(({invoices:d})=>{d.forEach((a,c)=>{S({num:c+1,name:a==null?void 0:a.created_dt,amount:`$${a==null?void 0:a.amount}`,status:a==null?void 0:a.status,invoice_link:a==null?void 0:a.link}),t.classList.remove("hide")})})};ee();})();
