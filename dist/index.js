...
<!--                <span id="progress" class="loader"></span>-->
        ...
        `,(r=document.querySelector("#submit"))==null||r.addEventListener("click",()=>{this.completePayment(e)})}async createPaymentIntent(e){fetch(`${y}/aion/payments/config`).then(t=>t.json()).then(t=>{re(t.stripe_public_key).then(n=>{this.stripe=n,this.is_test_mode=t.stripe_public_key.includes("test"),fetch(`${y}/aion/payments/create_intent`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${this.checkUserToken()}`},body:JSON.stringify({product_id:this.is_test_mode?e.test_id:e.id})}).then(s=>s.json()).then(s=>{var a;if(s.code===10)throw console.warn(s.message),alert(s.message),window.location.href=`https://${window.location.host}/`,new Error(s.message);this.elements=this.stripe.elements({clientSecret:s.client_secret});let r=this.elements.create("payment");(a=document.querySelector("#progress-loader"))==null||a.classList.add("hide"),document.querySelector("#submit").style.visibility="visible",r.mount("#payment-element")}).catch(s=>{console.error("Something wrong when create a payment intent",s),alert("Something wrong when create a payment. Please try again.")})})})}checkUserToken(){let e=localStorage.getItem("_ms-mid");if(!e)throw new Error("User token not found");return e}async initPaymentForm(e){console.log("Init payment form...");let t=await this.initStripe();if(this.stripe=t,!t)return;let n={theme:"flat",variables:{colorPrimary:"#30313d",colorText:"#30313d"},roles:{".TermsText":{hide:!0}}},s={},r="auto",a=await this.getClientSecret(e),d=t.elements({clientSecret:a,appearance:n,loader:r});this.elements=d;let v=d.create("payment",s).mount("#payment-element")}async getClientSecret(e){return fetch(`${y}/aion/payments/create_intent`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${this.checkUserToken()}`},body:JSON.stringify({product_id:this.is_test_mode?e.test_id:e.id})}).then(t=>t.json()).then(t=>{if(t.code===10)throw console.warn(t.message),alert(t.message),window.location.href=`https://${window.location.host}/`,new Error(t.message);return t.client_secret})}initStripe(){return console.log("Init stripe..."),fetch(`${y}/aion/payments/config`).then(e=>e.json()).then(e=>(this.is_test_mode=e.stripe_public_key.includes("test"),re(e.stripe_public_key).then(t=>t))).catch(e=>{console.error("Something wrong when init stripe",e)})}async completePayment(e){console.log("Complete payment...");let t=document.querySelector("#submit");if(t.innerHTML.includes("Payments attempt")){window.location.href=`https://${window.location.host}/#plans`;return}let n=t.innerHTML,s=null,r=null;for(let a=1;a<=5&&(this.showBlinkMessage(`Payments attempt ${a}...`,t),s=await this.stripe.confirmPayment({elements:this.elements,confirmParams:{return_url:`https://${window.location.host}/dashboard/settings?payment_success=${e.name}`}}).then(d=>d.error?(r=d.error.message,"error"):(console.log(d),"success")),t.innerHTML=n,console.log("result: ",s),!(s==="false"||s==="success"));a++);console.log(s,r),s==="error"&&alert(r)}async cancelSubscription(){fetch(`${y}/aion/payments/cancel_subscription`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${this.checkUserToken()}`}}).then(e=>e.json()).catch(e=>{console.error("Something wrong when create a checkout session",e)})}async getSubscriptionInfo(){try{let e=await fetch(`${y}/aion/payments/subscription`,{method:"GET",headers:{"Content-Type":"application/json",Authorization:`Bearer ${this.checkUserToken()}`}});return await this.handleResponse(e)}catch{return null}}async handleResponse(e){if(!e.ok){let t=await e.json();throw{status:e.status,message:t}}if(e.status!==204)return await e.json()}isValidCreditCardNumber(e){if(e=e.replace(/\s/g,"").split("").reverse().join(""),!/^[0-9]{13,19}$/.test(e))return!1;let t=0;for(let n=0;n<e.length;n++){let s=parseInt(e[n]);n%2===1&&(s*=2,s>9&&(s-=9)),t+=s}return t%10===0}};var _e=new F,f=class{constructor(e){let t=l.getToken(),n=`${y}/aion/${e}`;this.client=new D(n,t)}static sendVerificationEmail(){let{client:e}=f.getInstance();return e.get("email_verification",{})}static getInstance(e="users"){return f.instance||(f.instance=new f(e)),f.instance}static async fetchRequests(e=0,t=10){try{let{client:n}=f.getInstance(),s=`data?filters=requests&offset=${e}&limit=${t}`;return await n.get(s).then(r=>r.requests.array)}catch(n){return console.error("getRequests:",n),[]}}static async fetchUsageApi(){try{let{client:e}=f.getInstance(),t="data?filters=all&offset=0&limit=1";return await e.get(t).then(n=>n.api)}catch(e){return console.error("fetchUsageApi:",e),[]}}static async signUp(){try{let{client:e}=f.getInstance();return await e.post("sign_up",{}).then(()=>!1).catch(t=>{throw t})}catch(e){throw console.error("signUp:",e),e}}static async login(){try{let{client:e}=f.getInstance();return await e.get("login")}catch(e){console.error("login:",e)}}static async delete(){try{let{client:e}=f.getInstance();return l.removeAuth(),await e.delete(""),!0}catch(e){console.error("delete:",e)}}static async fetchApiToken(){try{let{client:e}=f.getInstance();return await e.post("api_token",{})}catch(e){console.error("fetchApiToken:",e),console.error("fetchApiToken:",e)}}static async refreshApiToken(){try{let{client:e}=f.getInstance();return await e.patch("api_token",{})}catch(e){console.error("refreshApiToken:",e)}}static async fetchSubscriptionData(){let{client:e}=f.getInstance();try{let t="data?filters=all&offset=0&limit=1";return await e.get(t).then(n=>n)}catch(t){console.error("fetchSubscriptionData:",t)}}static async subscriptionInfo(){var d,v,p,x,L,R,M,k,I,U,b;let e=document.querySelector("#plan-info"),t=document.querySelector("#usage-info"),n=document.querySelector("#btn-cancel-plan");e.classList.add("hide"),t.classList.add("hide"),n.classList.add("hide");let s=[f.fetchSubscriptionData(),_e.getSubscriptionInfo()],[r,a]=await Promise.all(s);console.log(r),console.log(a),r!=null&&r.plan||(e.innerHTML=`You're on the <span class="text-color-green">Free</span> plan. You have limits of 20 & 100 checks per month via web & API, respectively. If you need more, check out our <a class="text-color-green" href='https://${window.location.host}/#plans' >plans</a>.`,t.innerHTML=`You have used ${(((d=r==null?void 0:r.requests)==null?void 0:d.total)||0)-(((p=(v=r==null?void 0:r.api)==null?void 0:v.usage)==null?void 0:p.daily)||0)} of 20 checks via web and ${((L=(x=r==null?void 0:r.api)==null?void 0:x.usage)==null?void 0:L.daily)||0} of 100 checks via API.`,e.classList.remove("hide"),t.classList.remove("hide")),(r==null?void 0:r.plan.requests_limits.quantity)===1e3&&(e.innerHTML=`You're on the <span class="text-color-green">Base</span> plan. You have limits of 1000 requests for both web & API.`,t.innerHTML=`You have used ${((R=r==null?void 0:r.requests)==null?void 0:R.total)||0} of 1000 checks via both web API.`,(M=a==null?void 0:a.subscription.meta)!=null&&M.was_canceled?(n.classList.add("hide"),t.innerHTML=t.innerHTML+` Your subscription has been canceled, expires on ${new Date(a==null?void 0:a.subscription.expiration_dt).toLocaleDateString()}.`):n.classList.remove("hide"),e.classList.remove("hide"),t.classList.remove("hide")),(r==null?void 0:r.plan.requests_limits.quantity)===1e4&&(e.innerHTML=`You're on the <span class="text-color-green">PRO</span> plan. You have limits of 10000 requests for both web & API.`,t.innerHTML=`You have used ${((k=r==null?void 0:r.requests)==null?void 0:k.total)||0} of 10000 checks via both web & API.`,(U=(I=a==null?void 0:a.subscription)==null?void 0:I.meta)!=null&&U.was_canceled?(n.classList.add("hide"),t.innerHTML=t.innerHTML+` Your subscription has been canceled, expires on ${new Date(a==null?void 0:a.subscription.expiration_dt).toLocaleDateString()}.`):n.classList.remove("hide"),e.classList.remove("hide"),t.classList.remove("hide")),(r==null?void 0:r.plan.requests_limits.quantity)>1e4&&(e.innerHTML=`You're on the <span class="text-color-green">Custom</span> plan. You have limits of ${r==null?void 0:r.plan.requests_limits.quantity} requests for both web & API.`,t.innerHTML=`You have used ${((b=r==null?void 0:r.requests)==null?void 0:b.total)||0} of 10000 checks via both web & API.`,n.innerHTML="Contact US  to update your plan.",n.classList.remove("hide"),e.classList.remove("hide"),t.classList.remove("hide")),n.addEventListener("click",async()=>{(r==null?void 0:r.plan.requests_limits.quantity)>1e4?window.location.href=`https://${window.location.host}/contact-us`:confirm("Are you sure you want to cancel your subscription?")?(alert("Your subscription has been canceled."),_e.cancelSubscription().then(S=>{console.log(S)}),window.location.href=`https://${window.location.host}/#plans`):alert("Your subscription has not been canceled.")})}static async getUserInfo(){let{client:e}=f.getInstance();try{return await e.get("")}catch(t){console.error("getUserInfo:",t)}}static async getTransactions(){let{client:e}=new f("payments");try{return await e.get("invoices")}catch(t){console.error("getTransactions:",t)}}},C=f;C.instance=null;var $=class{constructor(){}static getUserInfo(){return C.getUserInfo()}static isAuth(){return localStorage.getItem($.token_key)!==null}static setAuth(){localStorage.setItem($.key,"true")}static removeAuth(){localStorage.removeItem($.key)}static async init(){if(localStorage.getItem("_aion_in")===null)try{await C.signUp(),$.setAuth(),await C.login(),localStorage.setItem("_aion_in","true")}catch(e){console.log(e)}}static getToken(){var e;return(e=localStorage.getItem("_ms-mid"))!=null?e:""}static isExpiredToken(){let e=$.getToken();if((e==null?void 0:e.length)>0){let t=Se(e),n=Date.now()/1e3;return t.exp<n}return!0}static checkAuth(e){if(!$.isAuth()){let t=document.getElementById("sign-up");return t.style.display="flex",t.style.zIndex=100,e(),!0}return!1}static async sendVerifiedEmail(){console.log("sendVerifiedEmail"),await C.sendVerificationEmail()}},l=$;l.key="isSignUp",l.token_key="_ms-mid";var q=class{constructor(){let e=l.getToken(),t=`${y}/aion/ai-generated`;this.client=new D(t,e)}static getInstance(){return q.instance||(q.instance=new q),q.instance}static async getReportsByBinary(e){let{client:t}=q.getInstance();try{let n=new FormData;return n.append("binary",e,"uploaded-file.png"),await t.postBinary("reports/binary",n)}catch(n){n.status===402&&alert("Please verify your email to continue using the service"),n.status===429&&(alert(`It looks like you have reached your plan limit of ${n.message.msg.current_limit} requests. To continue, please upgrade to a new plan.`),window.location.href=`https://${window.location.host}/#plans`),console.error("Error getReportsByBinary:",n)}}static async getReportsByUrl(e){let{client:t}=q.getInstance();try{let n=`reports/url?url=${e}`;return await t.post(n,{})}catch(n){n.status===402&&alert("Please verify your email to continue using the service"),n.status===429&&(alert(`It looks like you have reached your plan limit of ${n.message.msg.current_limit} requests. To continue, please upgrade to a new plan.`),window.location.href=`https://${window.location.host}/#plans`),console.error("getReportsByUrl:",n)}}static async getAudioVerdict(e){let{client:t}=q.getInstance();try{let n=new FormData;return n.append("file",e),await t.postBinary("reports/audio/binary",n)}catch(n){n.status===402&&alert("Please verify your email to continue using the service"),n.status===429&&(alert(`It looks like you have reached your plan limit of ${n.message.msg.current_limit} requests. To continue, please upgrade to a new plan.`),window.location.href=`https://${window.location.host}/#plans`),console.error("Error getAudioVerdict:",n)}}static async getYoutubeVerdict(e){let{client:t}=q.getInstance();try{let n={url:e};return await t.post("reports/audio/link",n)}catch(n){n.status===402&&alert("Please verify your email to continue using the service"),n.status===429&&(alert(`It looks like you have reached your plan limit of ${n.message.msg.current_limit} requests. To continue, please upgrade to a new plan.`),window.location.href=`https://${window.location.host}/#plans`),console.error("Error getYoutubeVerdict:",n)}}static async getPdetReportByUrl(e){let{client:t}=q.getInstance();try{let n={object:e};return await t.post("reports/person_detection/url",n)}catch(n){n.status===402&&alert("Please verify your email to continue using the service"),n.status===429&&(alert(`It looks like you have reached your plan limit of ${n.message.msg.current_limit} requests. To continue, please upgrade to a new plan.`),window.location.href=`https://${window.location.host}/#plans`),console.error("Error getYoutubeVerdict:",n)}}static async getPdetReportByBinary(e){let{client:t}=q.getInstance();try{let n=new FormData;return n.append("file",e,"uploaded-file.png"),await t.postBinary("reports/person_detection/binary",n)}catch(n){n.status===402&&alert("Please verify your email to continue using the service"),n.status===429&&(alert(`It looks like you have reached your plan limit of ${n.message.msg.current_limit} requests. To continue, please upgrade to a new plan.`),window.location.href=`https://${window.location.host}/#plans`),console.error("Error getReportsByBinary:",n)}}},_=q;_.instance=null;var V=class{constructor(){}static async getReportsByBinary(e,t){let n=`${y}/results/api/detector/reports/raw?source=web&user_id=${t}`,s=new FormData;s.append("binary",e,"file_name.png");let r={method:"POST",headers:{Accept:"application/json",Authorization:`Bearer ${l.getToken()}`},body:s};return await fetch(n,r).then(a=>a.json())}static async getReportsByUrl(e,t){let n=`${y}/results/api/detector/reports/json?source=web&user_id=${t}`,s={method:"POST",headers:{Accept:"application/json","Content-Type":"application/json",Authorization:`Bearer ${l.getToken()}`},body:JSON.stringify({object:e})};return await fetch(n,s).then(r=>r.json())}static async sendFeedback(e,t,n,s=!1){let r={is_proper_predict:t,comment:n},a=`${y}/results/api/detector/reports/result/${e}`,d={method:"PUT",body:JSON.stringify(r),headers:{Accept:"application/json","Content-Type":"application/json"}};(s||!l.isExpiredToken())&&(a=`${y}/aion/ai-generated/reports/${e}`,d={method:"PATCH",body:JSON.stringify(r),headers:{Accept:"application/json","Content-Type":"application/json"}}),await fetch(a,d).then(v=>v.json()).then(v=>console.log(v)).catch(v=>console.error(v))}static async getAudioVerdict(e){let t=`${y}/aion/ai-generated/reports/audio/binary`,n=new FormData;return n.append("file",e),await fetch(t,{method:"POST",headers:{Accept:"application/json",ContentType:"multipart/form-data"},body:n}).then(r=>r.json())}static async getYoutubeVerdict(e){let t=`${y}/aion/ai-generated/reports/audio/link`,n={method:"POST",headers:{Accept:"application/json","Content-Type":"application/json"},body:JSON.stringify({url:e})};return await fetch(t,n).then(s=>s.json())}};var G=class{constructor(){}static isLimitExceeded(){return!!l.isExpiredToken()}static increment(){let e=localStorage.getItem(G.key),t=e===null?1:Number(e)+1;localStorage.setItem(G.key,t.toString())}},A=G;A.key="requestCount";var T=class{static async getReportsByBinary(e,t){return l.isExpiredToken()?await V.getReportsByBinary(e,t):await _.getReportsByBinary(e)}static async getReportsByUrl(e,t){return l.isExpiredToken()?await V.getReportsByUrl(e,t):await _.getReportsByUrl(e)}static async getAudioVerictByFile(e){return await _.getAudioVerdict(e)}static async getAudioVerictMock(e){let n=await((s,r)=>new Promise(a=>{setTimeout(()=>{a(r)},s)}))(1500,e);return JSON.parse(`{
            "id": "41994fdd-0161-43a9-b873-581eccbe6d72",
            "report": {
                "version": "0.0.0",
                "verdict": ${n}
            }
        }`)}static async getYoutubeVerict(e){return l.isExpiredToken()?await V.getYoutubeVerdict(e):await _.getYoutubeVerdict(e)}static async getPdetReportByUrl(e){return await _.getPdetReportByUrl(e)}static async getPdetReportByBinary(e){return console.log("getPdetReportByBinary"),await _.getPdetReportByBinary(e)}static async sendFeedback(e,t,n,s=!1){return await V.sendFeedback(e,t,n,s)}};var oe=document.querySelector("#contact-us-submit-button"),nt=document.querySelector("#name"),st=document.querySelector("#E-Mail"),rt=document.querySelector("#Note"),ot=document.querySelector("#Company");oe&&(oe.classList.remove("is-disabled"),oe.addEventListener("click",async i=>{i.preventDefault();let e={name:nt.value,email:st.value,note:rt.value,company:ot.value};for(let t in e)if(t!=="company"&&e[t]===""){alert(`Please fill in all required fields ${t}`);return}console.log(e),fetch(`${y}/aion/system/post_message`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)}).then(t=>{t.ok?(alert("Thank you for your application! We will contact you shortly."),window.location.href="https://aiornot.com/"):alert("Something went wrong. Please try again.")})}));var Ie=()=>{var Ee,Le;let i=document.getElementById("image-tab"),e=document.getElementById("report-screen"),t=document.querySelector("#button-report-submit"),n=document.querySelector("#input-report-comment"),s=document.querySelector("#button-report_true"),r=document.querySelector("#button-report_false"),a=document.querySelector("#button-report_close"),d=document.querySelector("#url-error-message"),v=document.querySelector("#processing_cancel"),p=document.querySelector("#image-file-input"),x=document.querySelector("#ai-or-not-current-image"),L=document.querySelector("#empty-preview-img"),R=document.querySelector("#nsfw-preview-img"),M=document.querySelector("#input-error-text"),k=document.querySelector("#ai-or-not_image-url"),I=document.querySelector("#image-url-aion-submit"),U=document.querySelector("#ai-or-not_dropzone"),b=document.querySelector("#ai-or-not_dropzone-text"),S=document.querySelector("#result-screen_col"),j=document.querySelector("#share-items-hide"),K=document.querySelector("#ai-or-not-dropzone-counter"),X=document.querySelector("#ai-or-not-dropzone-counter-w"),O=document.querySelectorAll("#image-test-image");document.querySelector("#w-node-_80502d56-29f7-2965-16f1-d6c6c4ebbd86-c4ebbd86").classList.add("hide"),j.classList.add("hide");function u(){return i.classList.contains("w--current")?"image":"audio"}let g,h,w,H;se();let Me=()=>{let o=document.querySelector("#button-report_false-text");o.classList.remove("hide"),o.textContent=o.getAttribute("report-button-text-default_reported"),r.classList.add("is-reported"),s.classList.add("hide")},He=()=>{let o=document.querySelector("#button-report_true-text");o.classList.remove("hide"),o.textContent=o.getAttribute("report-button-text-default_reported"),s.classList.add("is-reported"),r.classList.add("hide")},$e=()=>{n.value="";let o=document.querySelector("#button-report_true-text"),m=document.querySelector("#button-report_false-text");m.classList.add("hide"),o.classList.remove("hide"),o.textContent=o.getAttribute("report-button-text-default"),m.textContent=m.getAttribute("report-button-text-default"),s.classList.remove("is-reported"),r.classList.remove("is-reported"),s.classList.remove("hide"),r.classList.remove("hide")},ce=o=>{H=o;let m=document.querySelector('[fs-socialshare-element="url"]'),E=`${l.isExpiredToken()?`${W}/aiornot/`:`${W}/aiornot/users/`}${o}`;m.textContent=E,document.querySelectorAll(".result-screen_share-item").forEach(ne=>{ne.setAttribute("data-url",E)})},le=()=>{b.textContent="We support jpeg, png, webp, gif, tiff, bmp. 10 Mb of maximum size.",b.classList.remove("text-color-red"),M.textContent="Something went wrong. Try again.",d.classList.add("hide")},de=()=>{d.classList.remove("hide")},Re=()=>{d.classList.add("hide")},Y=()=>{S.classList.contains("hide")?(b.textContent="File is too large (max 10 MB)",b.classList.add("text-color-red")):(M.textContent="File is too large (max 10 MB)",d.classList.remove("hide"))};p==null||p.addEventListener("change",()=>{let o=p==null?void 0:p.files[0].size,m=10*1024*1024;o>m?(w=!1,Y()):(w=!0,le())});let Ue=()=>{document.querySelector("#processing-screen").classList.add("hide"),b.classList.add("error"),U.classList.add("red-border"),b.textContent="Something went wrong. Try again."},ue=()=>{b.classList.remove("error"),U.classList.remove("red-border"),b.textContent="We support jpeg, png, webp, gif, tiff, bmp. 10 Mb of maximum size."},z=()=>{document.querySelector("#choose-file-row").classList.add("hide"),document.querySelector("#legal-tip").classList.remove("hide"),document.querySelector("#processing-screen").classList.add("hide"),document.querySelector("#hero-home_title-description").classList.remove("hide"),document.querySelector("#hero-home_gallery").classList.remove("hide"),document.querySelector("#ai-or-not_dropzone").classList.remove("hide"),document.querySelector("#hero-home_drop-zone-divider").classList.remove("hide"),document.querySelector("#result-screen_col").classList.add("hide"),document.querySelector("#result-screen_image-wrapper").classList.add("hide"),x.classList.add("hide"),L.classList.remove("hide"),R.classList.remove("hide")},me=()=>{$e(),x.src="",Re(),M.textContent="Something went wrong. Try again.",document.querySelector("#choose-file-row").classList.remove("hide"),document.querySelector("#legal-tip").classList.add("hide"),document.querySelector(".processing-screen_triggers_5").click(),document.querySelector("#processing-screen").classList.remove("hide"),document.querySelector(".processing-screen_triggers_1").click(),document.querySelector("#hero-home_title-description").classList.add("hide"),document.querySelector("#hero-home_gallery").classList.add("hide"),document.querySelector("#ai-or-not_dropzone").classList.add("hide"),document.querySelector("#hero-home_drop-zone-divider").classList.add("hide"),document.querySelector("#result-screen_col").classList.remove("hide"),document.querySelector("#result-screen_image-wrapper").classList.remove("hide")};function pe(o=!1){o?(R.classList.remove("hide"),x.classList.add("hide"),L.classList.add("hide"),j.classList.add("hide")):(R.classList.add("hide"),x.classList.remove("hide"),L.classList.add("hide")),document.querySelector(".processing-screen_triggers_3").click(),document.querySelector("#processing-screen").classList.add("hide"),document.querySelector(".processing-screen_triggers_5").click(),p.value="",document.querySelector("#ai-or-not_image-url").value=""}let he=o=>{o==="unknown"?(document.getElementById("title-human").innerHTML="Sorry, but in this case we can't really say if it's AI or Not",document.getElementById("ai-or-not_result-message-50").classList.remove("hide"),document.getElementById("ai-or-not_result-message").classList.add("hide"),document.getElementById("ai-or-not_result-message-50").innerHTML="Probly the uploaded image has most likely been modified or compressed",document.getElementById("title-human").classList.remove("hide"),document.getElementById("title-ai").classList.add("hide")):(document.getElementById("title-ai").innerHTML=`This is likely <span class="text-color-green">AI</span> <div style="font-size: 1rem; color: #FFFFFFB3; font-family: Space Grotesk, sans-serif;">
...
<span> Free Research Preview. AI or Not may produce inaccurate results </span>
</div>`,document.getElementById("ai-or-not_result-message-50").classList.add("hide"),document.getElementById("ai-or-not_result-message").classList.remove("hide"),document.querySelector("#ai-or-not_model-name").textContent=o,o==="ai"?(document.getElementById("title-human").classList.add("hide"),document.getElementById("title-ai").classList.remove("hide")):(document.getElementById("title-human").classList.remove("hide"),document.getElementById("title-ai").classList.add("hide")))},ye=async()=>{if(A.isLimitExceeded()){let o=document.getElementById("sign-up");o.style.display="flex",o.style.zIndex=100,z()}else d.classList.add("hide"),me(),await T.getReportsByUrl(g,Z).then(o=>{A.increment(),ce(o.id),x.src=g,he(o.verdict),pe(o.nsfw_detected)}).catch(o=>{S.classList.contains("hide")?de():(de(),z())})},P=document.body,lt=document.querySelector("#dropzone-fullscreen_message-tip"),dt=document.querySelector("#dropzone-fullscreen_message-format");P==null||P.addEventListener("dragover",function(o){o.preventDefault(),document.querySelector(".dropzone-fullscreen").classList.remove("hide")}),P==null||P.addEventListener("dragleave",function(o){o.preventDefault(),document.querySelector(".dropzone-fullscreen").classList.add("hide")}),P==null||P.addEventListener("drop",async function(o){if(u()!=="image")return;o.preventDefault(),document.querySelector(".dropzone-fullscreen").classList.add("hide");let m=o.dataTransfer.files[0],B=m.size,E=10*1024*1024;B>E?(w=!1,Y()):(w=!0,le()),w==!0?await ge(m):Y()}),p==null||p.addEventListener("change",async o=>{if(w==!0){let m=p;console.log(m);let B=m.files[0];await ge(B)}else Y()});let ge=async o=>{if(me(),A.isLimitExceeded()){let E=document.getElementById("sign-up");E.style.display="flex",E.style.zIndex=100,z();return}let m=document.querySelector("#ai-or-not-current-image"),B=URL.createObjectURL(o);m.setAttribute("src",B),x.classList.remove("hide"),L.classList.add("hide"),await T.getReportsByBinary(o,Z).then(E=>{A.increment(),ce(E.id),ue(),he(E.verdict),pe(E.nsfw_detected)}).catch(E=>{Ue(),z()})};v==null||v.addEventListener("click",function(){ue(),z()}),(Ee=document.querySelector("#ai-or-not_dropzone"))==null||Ee.addEventListener("click",function(){u()==="image"&&(l.checkAuth(z)||(h="screen_home",p.click()))}),(Le=document.querySelector("#choose-file-row"))==null||Le.addEventListener("click",function(){u()==="image"&&(h="screen_result",p.click())}),I==null||I.addEventListener("click",()=>{k.value!=""&&(g=k.value,ye())});let Q=document.querySelector("#ai-or-not_image-url");Q==null||Q.addEventListener("keypress",function(o){o.key==="Enter"&&k.value!=""&&(g=k.value,ye())}),O.forEach(o=>{o==null||o.addEventListener("click",()=>{let m=o.getAttribute("test-image-url");document.querySelector("#ai-or-not_image-url").value=m,document.querySelector("#image-url-aion-submit").click(),document.querySelector("#ai-or-not_image-url").value=""})}),s==null||s.addEventListener("click",()=>{He(),T.sendFeedback(H,!0,"")}),r==null||r.addEventListener("click",()=>{}),a==null||a.addEventListener("click",()=>{}),t==null||t.addEventListener("click",()=>{T.sendFeedback(H,!1,n.value),Me()}),document==null||document.addEventListener("keydown",function(o){o.code==="Escape"&&e.style.display!=="none"&&a.click()}),n==null||n.addEventListener("change",()=>{n.value!=""?t.classList.remove("is-disabled"):t.classList.add("is-disabled")}),n==null||n.addEventListener("input",()=>{n.value!=""?t.classList.remove("is-disabled"):t.classList.add("is-disabled")});let fe=document.querySelector("#ai-or-not_image-url"),ve=document.querySelector("#image-url-aion-submit");fe.addEventListener("input",function(){let o=fe.value.trim();Ve(o)?ve.classList.remove("is-disabled"):ve.classList.add("is-disabled")});let Ve=o=>{try{return new URL(o),!0}catch{return!1}},ee=document.getElementById("close-sign-up");ee==null||ee.addEventListener("click",()=>{let o=document.getElementById("sign-up");o.style.display="none",o.style.zIndex=0});let we=document.querySelector("#image-quotas");l.isAuth()&&C.fetchSubscriptionData().then(o=>{var m,B;if(o){let{quantity:E}=((m=o.plan)==null?void 0:m.requests_limits)||{quantity:20},{total:te}=o.requests;if(!o.plan)try{te-=((B=o.api.usage)==null?void 0:B.daily)||0}catch(ne){console.log(ne)}we.innerHTML=`
            <div style="margin-top: 20px; font-size: 1rem; color: white">
            <span">
                Available ${E-te} from ${E} requests 
            </span>
            </div>`}else we.textContent=""})};var ie=()=>{let i=new F,e=document.querySelector("#bt-pay-free"),t=document.querySelector("#bt-pay-basic"),n=document.querySelector("#bt-pay-pro"),s=document.querySelector("#bt-pay-enterprice");e==null||e.addEventListener("click",()=>{localStorage.getItem("_ms-mid")?window.location.href=`https://${window.location.host}/`:window.location.href=`https://${window.location.host}/signup`}),t==null||t.addEventListener("click",()=>{if(!localStorage.getItem("_ms-mid")){window.location.href=`https://${window.location.host}/signup`;return}i.checkout(i.PRODUCT_ID_BASE_PLAN)}),n==null||n.addEventListener("click",()=>{if(!localStorage.getItem("_ms-mid")){window.location.href=`https://${window.location.host}/signup`;return}i.checkout(i.PRODUCT_ID_PRO_PLAN)}),s==null||s.addEventListener("click",()=>{window.location.href=`https://${window.location.host}/contact-us`})};var ae=()=>{document.querySelector("#processing-screen").classList.remove("hide"),document.querySelector(".processing-screen_triggers_5").click(),document.querySelector(".processing-screen_triggers_1").click()},J=()=>{document.querySelector("#processing-screen").classList.add("hide")},Pe=()=>{window.location.href="/sign-in"};var it=document.getElementById("image-tab"),at=document.getElementById("audio-tab"),ct=document.getElementById("pdet-tab");function Be(){if(it.classList.contains("w--current"))return"image";if(at.classList.contains("w--current"))return"audio";if(ct.classList.contains("w--current"))return"pdet"}var Ae=()=>{var X,O;window.location.host.includes("webflow")&&((X=document.querySelector("#pdet-tab"))==null||X.classList.remove("hide"));let i=document.querySelectorAll("#pdet-test-image"),e=document.querySelector("#pdet-file-input"),t=document.querySelector("#aion-pdet-current-image"),n=document.querySelector("#pdet-empty-preview-img"),s=document.querySelector("#pdet-nsfw-preview-img"),r=document.querySelector("#aion-pdet-image-url"),a=document.querySelector("#aion-pdet-url-submit"),d=document.querySelector("#aion-pdet-dropzone"),v=document.querySelector("#aion-pdet-dropzone-text"),p,x,L,R,M=()=>{var c,u,g,h,w,H;(c=document.querySelector("#pdet-hero-home-title-description"))==null||c.classList.add("hide"),(u=document.querySelector("#pdet-hero-home-gallery"))==null||u.classList.add("hide"),(g=document.querySelector("#aion-pdet-dropzone"))==null||g.classList.add("hide"),(h=document.querySelector("#pdet-button-report-true"))==null||h.classList.add("hide"),(w=document.querySelector("#pdet-button-report-false"))==null||w.classList.add("hide"),(H=document.querySelector("#pdet-hero-home_drop-zone-divider"))==null||H.classList.add("hide")},k=()=>{d.classList.add("red-border"),v.textContent="Something went wrong. Try again.",v.classList.add("error")};e==null||e.addEventListener("change",()=>{let c=e==null?void 0:e.files[0].size,u=10*1024*1024;c>u?L=!1:L=!0});function I(c=!1,u,g){var h,w;M(),J(),c||(s.classList.add("hide"),t.classList.remove("hide"),n.classList.add("hide"),(h=document.querySelector("#result-screen-pdet-wrapper"))==null||h.classList.remove("hide"),t.classList.remove("hide"),t.src=g,(w=document.querySelector("#pdet-result-screen-col"))==null||w.classList.remove("hide"),document.getElementById("pdet-title-human").classList.remove("hide"),document.getElementById("pdet-title-human").innerHTML=`This is likely <span class="text-color-green">${u}</span><div style="font-size: 1rem; color: #FFFFFFB3; font-family: Space Grotesk, sans-serif;">
<span> Free Research Preview. AI or Not may produce inaccurate results </span>
</div>`),r.value=""}let U=async()=>{console.log("pastedUrl",p),await T.getPdetReportByUrl(p).then(c=>{I(c.nsfw_detected,c!=null&&c.verdict?"AI":"Human",c.url)}).catch(c=>{J(),k()})},b=async c=>{ae();let u=document.querySelector("#ai-or-not-current-image"),g=URL.createObjectURL(c);u.setAttribute("src",g),t.classList.remove("hide"),n.classList.add("hide"),await T.getPdetReportByBinary(c).then(h=>{I(h.nsfw_detected,h!=null&&h.verdict?"AI":"Human",h.url)}).catch(h=>{J(),k()})},S=document.body;S==null||S.addEventListener("dragover",function(c){c.preventDefault(),document.querySelector(".dropzone-fullscreen").classList.remove("hide")}),S==null||S.addEventListener("dragleave",function(c){c.preventDefault(),document.querySelector(".dropzone-fullscreen").classList.add("hide")}),S==null||S.addEventListener("drop",async function(c){if(Be()!=="pdet")return;c.preventDefault(),document.querySelector(".dropzone-fullscreen").classList.add("hide");let u=c.dataTransfer.files[0],g=u.size,h=10*1024*1024;g>h?L=!1:L=!0,L==!0?await b(u):k()}),e==null||e.addEventListener("change",async c=>{if(L==!0){let g=e.files[0];await b(g)}else console.log("fileSizeAllow"),k()}),d==null||d.addEventListener("click",function(){l.checkAuth(Pe)||(x="screen_home",e.click())}),(O=document.querySelector("#choose-file-row"))==null||O.addEventListener("click",function(){x="screen_result",e.click()}),a==null||a.addEventListener("click",()=>{ae(),r.value!==""&&(p=r.value,U())}),r.addEventListener("input",function(){let c=r.value.trim();j(c)?a.classList.remove("is-disabled"):a.classList.add("is-disabled")});let j=c=>{try{return new URL(c),!0}catch{return!1}};i.forEach(c=>{c.addEventListener("click",()=>{let u=c.getAttribute("test-image-url");r.value=u,a.click(),r.value=""})});let K=document.querySelector("#image-quotas");l.isAuth()&&C.fetchSubscriptionData().then(c=>{var u,g;if(c){let{quantity:h}=((u=c.plan)==null?void 0:u.requests_limits)||{quantity:20},{total:w}=c.requests;if(!c.plan)try{w-=((g=c.api.usage)==null?void 0:g.daily)||0}catch(H){console.log(H)}console.log(c),K.innerHTML=`
            <div style="margin-top: 20px; font-size: 1rem; color: white">
            <span">
                Available ${h-w} from ${h} requests
            </span>
            </div>`}else K.textContent=""})};Ie();Ae();ie();})();
