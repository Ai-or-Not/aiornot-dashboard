...
@media (min-width: 576px) {
  ...
  .payment-form .card-details button {
    margin-top: 2em;
            ...
            `,W=class{constructor(){this.elements=null;this.stripe=null;this.home_element=document.querySelector("#home");this.PRODUCT_ID_BASE_9USD_PLAN={id:"base-plan",msg:"Base plan: $9/month",name:"Base plan",description:"300 requests per month",price:"$9",test_id:"base-plan"};this.PRODUCT_ID_PRO_PLAN={id:"pro-plan",msg:"Pro plan: $250/month",name:"Pro plan",description:"10,000 requests per month",price:"$250",test_id:"pro-plan"};this.is_test_mode=!1}showBlinkMessage(e,t){let n=document.createElement("style");n.innerHTML=Be,document.head.appendChild(n),t.innerHTML=`
...
<span> Free Research Preview. AI or Not may produce inaccurate results </span>
</div>`,document.getElementById("ai-or-not_result-message-50").classList.add("hide"),document.getElementById("ai-or-not_result-message").classList.remove("hide"),document.querySelector("#ai-or-not_model-name").textContent=r,r==="ai"?(document.getElementById("title-human").classList.add("hide"),document.getElementById("title-ai").classList.remove("hide")):(document.getElementById("title-human").classList.remove("hide"),document.getElementById("title-ai").classList.add("hide")))},fe=async()=>{if(H.isLimitExceeded()){let r=document.getElementById("sign-up");r.style.display="flex",r.style.zIndex=100,D()}else l.classList.add("hide"),he(),await _.getReportsByUrl(L,te).then(r=>{H.increment(),de(r.id),x.src=L,ge(r.verdict),ye(r.nsfw_detected),r.good_quality===!1&&Y()}).catch(r=>{v.classList.contains("hide")?me():(me(),D())})},$=document.body,ft=document.querySelector("#dropzone-fullscreen_message-tip"),vt=document.querySelector("#dropzone-fullscreen_message-format");$==null||$.addEventListener("dragover",function(r){r.preventDefault(),document.querySelector(".dropzone-fullscreen").classList.remove("hide")}),$==null||$.addEventListener("dragleave",function(r){r.preventDefault(),document.querySelector(".dropzone-fullscreen").classList.add("hide")}),$==null||$.addEventListener("drop",async function(r){if(u()!=="image")return;r.preventDefault(),document.querySelector(".dropzone-fullscreen").classList.add("hide");let y=r.dataTransfer.files[0],N=y.size,S=10*1024*1024;N>S?(A=!1,J()):(A=!0,ue()),A===!0?await ve(y):J()}),h==null||h.addEventListener("change",async r=>{if(A===!0){let y=h;console.log(y);let N=y.files[0];await ve(N)}else J()});let ve=async r=>{if(he(),H.isLimitExceeded()){let S=document.getElementById("sign-up");S.style.display="flex",S.style.zIndex=100,D();return}let y=document.querySelector("#ai-or-not-current-image"),N=URL.createObjectURL(r);y.setAttribute("src",N),x.classList.remove("hide"),w.classList.add("hide"),await _.getReportsByBinary(r,te).then(S=>{H.increment(),de(S.id),pe(),ge(S.verdict),ye(S.nsfw_detected),S.good_quality===!1&&Y()}).catch(S=>{je(),D()})};f==null||f.addEventListener("click",function(){pe(),D()}),(be=document.querySelector("#ai-or-not_dropzone"))==null||be.addEventListener("click",function(){u()==="image"&&(d.checkAuth(D)||(z="screen_home",h.click()))}),(Le=document.querySelector("#choose-file-row"))==null||Le.addEventListener("click",function(){u()==="image"&&(z="screen_result",h.click())}),B==null||B.addEventListener("click",()=>{C.value!==""&&(L=C.value,fe())});let se=document.querySelector("#ai-or-not_image-url");se==null||se.addEventListener("keypress",function(r){r.key==="Enter"&&C.value!==""&&(L=C.value,fe())}),c.forEach(r=>{r==null||r.addEventListener("click",()=>{let y=r.getAttribute("test-image-url");document.querySelector("#ai-or-not_image-url").value=y,document.querySelector("#image-url-aion-submit").click(),document.querySelector("#ai-or-not_image-url").value=""})}),s==null||s.addEventListener("click",()=>{Ve(),_.sendFeedback(j,!0,"")}),o==null||o.classList.add("hide"),a==null||a.classList.add("hide"),t==null||t.addEventListener("click",()=>{_.sendFeedback(j,!1,n.value),ze()}),document==null||document.addEventListener("keydown",function(r){r.code==="Escape"&&e.style.display!=="none"&&a.click()}),n==null||n.addEventListener("change",()=>{n.value!==""?t.classList.remove("is-disabled"):t.classList.add("is-disabled")}),n==null||n.addEventListener("input",()=>{n.value!==""?t.classList.remove("is-disabled"):t.classList.add("is-disabled")});let we=document.querySelector("#ai-or-not_image-url"),Ee=document.querySelector("#image-url-aion-submit");we.addEventListener("input",function(){let r=we.value.trim();Ne(r)?Ee.classList.remove("is-disabled"):Ee.classList.add("is-disabled")});let Ne=r=>{try{return new URL(r),!0}catch{return!1}},oe=document.getElementById("close-sign-up");oe==null||oe.addEventListener("click",()=>{let r=document.getElementById("sign-up");r.style.display="none",r.style.zIndex=0}),G(document.querySelector("#image-quotas"))};var le=()=>{let i=new W,e=document.querySelector("#bt-pay-free"),t=document.querySelector("#bt-pay-basic"),n=document.querySelector("#bt-pay-pro"),s=document.querySelector("#bt-pay-enterprice");e==null||e.addEventListener("click",()=>{localStorage.getItem("_ms-mid")?window.location.href=`https://${window.location.host}/`:window.location.href=`https://${window.location.host}/signup`}),t==null||t.addEventListener("click",()=>{if(!localStorage.getItem("_ms-mid")){window.location.href=`https://${window.location.host}/signup`;return}i.checkout(i.PRODUCT_ID_BASE_9USD_PLAN)}),n==null||n.addEventListener("click",()=>{if(!localStorage.getItem("_ms-mid")){window.location.href=`https://${window.location.host}/signup`;return}i.checkout(i.PRODUCT_ID_PRO_PLAN)}),s==null||s.addEventListener("click",()=>{window.location.href=`https://${window.location.host}/contact-us`})};var gt=()=>{$e()};gt();Re();Ue();le();})();
