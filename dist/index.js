...
<span> Free Research Preview. AI or Not may produce inaccurate results </span>
</div>`,document.getElementById("ai-or-not_result-message-50").classList.add("hide"),document.getElementById("ai-or-not_result-message").classList.remove("hide"),document.querySelector("#ai-or-not_model-name").textContent=r,r==="ai"?(document.getElementById("title-human").classList.add("hide"),document.getElementById("title-ai").classList.remove("hide")):(document.getElementById("title-human").classList.remove("hide"),document.getElementById("title-ai").classList.add("hide")))},fe=async()=>{if(A.isLimitExceeded()){let r=document.getElementById("sign-up");r.style.display="flex",r.style.zIndex=100,D()}else d.classList.add("hide"),se(),await _.getReportsByUrl(b,ee).then(r=>{A.increment(),ue(r.id),x.src=b,ge(r.verdict),ye(r.nsfw_detected),r.good_quality===!1&&N()}).catch(r=>{f.classList.contains("hide")?pe():(pe(),D())})},B=document.body,ft=document.querySelector("#dropzone-fullscreen_message-tip"),vt=document.querySelector("#dropzone-fullscreen_message-format");B==null||B.addEventListener("dragover",function(r){r.preventDefault(),document.querySelector(".dropzone-fullscreen").classList.remove("hide")}),B==null||B.addEventListener("dragleave",function(r){r.preventDefault(),document.querySelector(".dropzone-fullscreen").classList.add("hide")}),B==null||B.addEventListener("drop",async function(r){if(u()!=="image")return;r.preventDefault(),document.querySelector(".dropzone-fullscreen").classList.add("hide");let y=r.dataTransfer.files[0],F=y.size,S=10*1024*1024;F>S?(P=!1,G()):(P=!0,me()),P===!0?await ve(y):G()}),h==null||h.addEventListener("change",async r=>{if(P===!0){let y=h;console.log(y);let F=y.files[0];await ve(F)}else G()});let ve=async r=>{if(se(),A.isLimitExceeded()){let S=document.getElementById("sign-up");S.style.display="flex",S.style.zIndex=100,D();return}let y=document.querySelector("#ai-or-not-current-image"),F=URL.createObjectURL(r);y.setAttribute("src",F),x.classList.remove("hide"),w.classList.add("hide"),await _.getReportsByBinary(r,ee).then(S=>{A.increment(),ue(S.id),he(),ge(S.verdict),ye(S.nsfw_detected),S.good_quality===!1&&N()}).catch(S=>{je(),D()})};v==null||v.addEventListener("click",function(){he(),D()}),(Le=document.querySelector("#ai-or-not_dropzone"))==null||Le.addEventListener("click",function(){u()==="image"&&(l.checkAuth(D)||($="screen_home",h.click()))}),(be=document.querySelector("#choose-file-row"))==null||be.addEventListener("click",function(){u()==="image"&&($="screen_result",h.click())}),I==null||I.addEventListener("click",()=>{C.value!==""&&(b=C.value,fe())});let oe=document.querySelector("#ai-or-not_image-url");oe==null||oe.addEventListener("keypress",function(r){r.key==="Enter"&&C.value!==""&&(b=C.value,fe())}),c.forEach(r=>{r==null||r.addEventListener("click",()=>{let y=r.getAttribute("test-image-url");document.querySelector("#ai-or-not_image-url").value=y,document.querySelector("#image-url-aion-submit").click(),document.querySelector("#ai-or-not_image-url").value=""})}),s==null||s.addEventListener("click",()=>{Ve(),_.sendFeedback(V,!0,"")}),o==null||o.classList.add("hide"),a==null||a.classList.add("hide"),t==null||t.addEventListener("click",()=>{_.sendFeedback(V,!1,n.value),ze()}),document==null||document.addEventListener("keydown",function(r){r.code==="Escape"&&e.style.display!=="none"&&a.click()}),n==null||n.addEventListener("change",()=>{n.value!==""?t.classList.remove("is-disabled"):t.classList.add("is-disabled")}),n==null||n.addEventListener("input",()=>{n.value!==""?t.classList.remove("is-disabled"):t.classList.add("is-disabled")});let we=document.querySelector("#ai-or-not_image-url"),Ee=document.querySelector("#image-url-aion-submit");we.addEventListener("input",function(){let r=we.value.trim();Oe(r)?Ee.classList.remove("is-disabled"):Ee.classList.add("is-disabled")});let Oe=r=>{try{return new URL(r),!0}catch{return!1}},re=document.getElementById("close-sign-up");re==null||re.addEventListener("click",()=>{let r=document.getElementById("sign-up");r.style.display="none",r.style.zIndex=0}),Z(document.querySelector("#image-quotas"))};var de=()=>{let i=new Y,e=document.querySelector("#bt-pay-free"),t=document.querySelector("#bt-pay-basic"),n=document.querySelector("#bt-pay-pro"),s=document.querySelector("#bt-pay-enterprice");e==null||e.addEventListener("click",()=>{localStorage.getItem("_ms-mid")?window.location.href=`https://${window.location.host}/`:window.location.href=`https://${window.location.host}/signup`}),t==null||t.addEventListener("click",()=>{if(!localStorage.getItem("_ms-mid")){window.location.href=`https://${window.location.host}/signup`;return}i.checkout(i.PRODUCT_ID_BASE_9USD_PLAN)}),n==null||n.addEventListener("click",()=>{if(!localStorage.getItem("_ms-mid")){window.location.href=`https://${window.location.host}/signup`;return}i.checkout(i.PRODUCT_ID_PRO_PLAN)}),s==null||s.addEventListener("click",()=>{window.location.href=`https://${window.location.host}/contact-us`})};var gt=()=>{Me()};gt();Re();Ue();de();})();
