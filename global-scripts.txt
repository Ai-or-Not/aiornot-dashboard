...
class ElementCreator {
	...
	static fillCardControls(parentElement, item) {
		...
		shareButton.onclick = () => {
			...
			copyToClipboard(`https://results.aiornot.com/aiornot/users/${item.report_id}`)
			setTimeout(() => {
		...
		if (!item.hasOwnProperty('is_proper_predict')) {
			...
			likeButton.onclick = () => {
				...
				WrapperAIGeneratedService.sendFeedback(item.report_id, true, '')
			}
			...
			dislikeButton.onclick = () => {
				...
				WrapperAIGeneratedService.sendFeedback(item.report_id, false, '')
			}
